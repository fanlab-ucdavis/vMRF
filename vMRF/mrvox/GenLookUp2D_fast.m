...
for NbFile = 1:numel(FileIn)
    ...
    if Para
        ...
        for JobId = 1:size(JobSeq,1)
            ...
            for SimuIdx = JobSeq(JobId,1):JobSeq(JobId,2)
                if Cluster.MRL.On % Used the standalone version with the MRL routine
                    ...
                    createTask(job(JobId), @VoxelSim2D_do_one_no_graphs, 2, {File,SeqFileName,SimuIdx,1});
                end
            end
            submit(job(JobId));
            ...
            if ~isempty(task_failed)
                ...
                while ~isempty(task_failed_remain) && (try_n_times > 0)
                    ...
                    for tas = 1:numel(task_failed_remain)
                        createTask(job_failed, @VoxelSim2D_do_one_no_graphs, 2, {File, SeqFileName, task_failed_remain(tas), 1});
                    end
                    submit(job_failed);
        ...
        for a = 1:NbSimu
            fprintf('Input %d/%d\n',a,NbSimu);
            [Sa, Sphi] = VoxelSim2D_do_one_no_graphs(File,SeqFileName,a,1);
            results{a,1} = Sa;
            results{a,2} = Sphi;
        end
    end
